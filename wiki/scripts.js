// return index as well as text so you can cycle through?
...
const findSnippet = (transcript, start = 0) => {
	const index = searchText.indexOf(transcript.toLowerCase(), start);

	if (index > -1) {
		const section = text.slice(index - 18, index + 18).trim();
		const trimmed = trimExtraLetters(section);
		return highlight(trimmed, transcript);
	} else {
		return '';
	}
}
...
function highlight(foundText, transcript) {
	const index = foundText.toLowerCase().indexOf(transcript.toLowerCase());

	if (index > -1) {
		return foundText.substring(0, index) + '<span class="highlight">' + foundText.substring(index, index + transcript.length) + '</span>' + foundText.substring(index + transcript.length);
	}
...
function setPageGray(words) {
	...
	words.style.setProperty('--text-color', textColor);
	const span = document.querySelector('.highlight')
	span.style.setProperty('--background-color', spanBackground);

	document.body.style.background = `hsl(0, 0%, ${hue}%)`;
}

const startSpeechDetection = (node) => {
	let interval;
	...
	recognition.addEventListener('result', e => {
		...
		if (lastWords == transcript || !transcript) return;
		if (!findSnippet(transcript)) return;
		lastWords = transcript;

		const findSnippetAndSetToPage = (transcript, node, start = randomStart()) => {
			const snippet = findSnippet(transcript, start);
			if (!snippet) return;
			node.innerHTML = `<h2>${snippet}</h2>`;
			return true;
		}
		...
		interval = setInterval(() => {
			if (first) {
				const wordsFound = findSnippetAndSetToPage(transcript, node, 0);
				if (wordsFound) {
					setPageGray(node);
					first = false;
				}
			}
			findSnippetAndSetToPage(transcript, node);
		}, 250)
